    ...
    def verify_schedule():
        ...
        for irrigation_scheduled in schedule_list:
            if day in irrigation_scheduled["days"]:
                activation_moment = datetime.strptime(
                    irrigation_scheduled["time"], "%H:%M:%S"
                ).time()
                duration = datetime.strptime(
                    irrigation_scheduled["duration"], "%H:%M:%S"
                ).time()
                stop_moment = timedelta(
                    hours=activation_moment.hour,
                    minutes=activation_moment.minute,
                    seconds=activation_moment.second,
                ) + timedelta(
                    hours=duration.hour,
                    minutes=duration.minute,
                    seconds=duration.second,
                )
                schedule.every().day.at(str(activation_moment)).do(
                    ScheduleIrrigationService.active_schedule,
                    irrigation_zone_id=irrigation_scheduled["irrigation_zone_id"],
                )
                schedule.every().day.at(str(stop_moment)).do(
                    ScheduleIrrigationService.active_schedule,
                    irrigation_zone_id=irrigation_scheduled["irrigation_zone_id"],
                )
