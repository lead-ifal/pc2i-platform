...
from app.constants.status_code import HTTP_BAD_REQUEST_CODE, HTTP_CREATED_CODE, HTTP_NOT_FOUND_CODE, HTTP_SUCCESS_CODE,HTTP_SERVER_ERROR_CODE
from app.constants.response_messages import ERROR_MESSAGE, READINGS_NOT_FOUND_MESSAGE, SUCCESS_MESSAGE, INTERNAL_SERVER_ERROR_MESSAGE
  ...
  def list_readings():
    ...
    try:
      ...
      if (irrigation_zone_id != None):
        isInvalidId = GlobalController.is_valid_mongodb_id(irrigation_zone_id) == False

        if (isInvalidId):
          return GlobalController.generate_response(HTTP_BAD_REQUEST_CODE, ERROR_MESSAGE)

        sensor_ids = []
        match_irrigation_zone = { 'irrigation_zone_id': ObjectId(irrigation_zone_id) }
        lookup_sensors = { 'from': 'sensors', 'localField': '_id', 'foreignField': 'culture_id', 'as': 'sensors' }
        ...
        if (len(sensor_ids) == 0):
          return GlobalController.generate_response(HTTP_NOT_FOUND_CODE, READINGS_NOT_FOUND_MESSAGE)
      ...
      lookup_sensors = { 'from': 'sensors', 'localField': 'sensor_id', 'foreignField': '_id', 'as': 'sensor' }
      lookup_cultures = { 'from': 'cultures', 'localField': 'sensor.culture_id', 'foreignField': '_id', 'as': 'culture' }
      fields = { 'value': 1, 'sensor_id': 1, 'sensor_type': '$sensor.type', 'irrigation_zone_id': '$culture.irrigation_zone_id' }
