                ...
                if GlobalController.is_valid_mongodb_id(user_id):
                    ...
                    if UserController.verify_prefilled_fields(body):
                        ...
                        if password_is_correct:
                            ...
                            user_data["password"] = UserController.encode_password(user_data["new_password"])
                            user_data["encrypted_email"] = UserController.encode_email(user_data["email"])

                            user_data.pop("new_password")

                            users.find_one_and_update({"_id": ObjectId(user_id)}, {"$set": user_data})
